...
## 2026-09-26 (2) — 컷신 오버스캔: 클립 그림의 흐린 테두리를 화면 밖으로

### 문제 (2026-09-26 Maker Play · 로컬 테스트 브랜치 `044cebd` · 사용자 판정 "통과 아님")

- 5종 궁 모두 `cutscene UI <SkillId> on` → 실측 길이 뒤 `off` · UI 그룹 런타임 순서 22(최상위 · `RevivePopupGroup` 21) · 맵 왼쪽 끝(x −8.84) · 가운데(x 0) 모두 HUD 가 컷신 아래. 그런데 **화면 가장자리마다 맵이 비쳤다** — 맵 끝과 가운데가 같다.
- 실측: 게임 보기 939×528 · UI 루트 1920×1080(StretchAll · ×0.489). `Cutscene` sprite 는 덮는 크기(W31 1436×840 → 1920×1123 · 가로 딱 맞고 위아래 약 10 보기 px 넘침)다. 프레임은 가장 큰 프레임에 대한 **제 크기**로 그려진다(늘이지 않는다 — 위 "모르는 것" 첫 줄의 "프레임마다 `RectSize` 에 맞춰 늘인다"는 틀렸다). M31 첫 프레임 480×820 은 클립 자체의 시작 섬광이다.
- **원인:** sprite 는 이미 화면을 덮는다. 클립 그림이 프레임 캔버스 안쪽에서 투명하게 흐려져, 그 테두리로 맵이 보인다.
- 변마다 비친 폭(보기 px · 컷신 없는 기준 화면과의 차이 · 95% 덮임 기준)과, 가운데 기준으로 키울 때 그 폭이 화면 밖으로 나가는 최소 배율(추정 `s ≥ (C/2) / (R/2 − B)` · C = 화면 · R = 덮는 크기 · B = 흐린 폭 + 이미 넘친 폭, UI px):

| 클립 | 왼 | 오 | 위 | 아래 | 최소 배율(추정) |
|---|---|---|---|---|---|
| W31 세이크리드 바스티온 | 12 | 21 | 6 | 25 | 1.10 (아래) |
| M31 프로즌 라이트닝 | 32 | 26 | 20 | 7 | 1.08 (위) |
| A31 파이널 에임 | 12 | 20 | 4 | 27 | 1.11 (아래) |
| T31 일도양단 | 12 | 17 | 7 | 10 | 1.04 (아래) |
| P31 드레드노트 | – | – | – | – | 프레임이 어두워 차이로 못 잼 |

### 수정

| 파일 | 변경 |
|---|---|
| `SkillExecutors.mlua` | `property number CutsceneOverscan = 1.08` — `StartCutsceneUI` 가 덮는 배율(`max(가로비, 세로비)`)에 곱한다(가운데 기준 · 네 변 같은 비율 · 0 이하면 1). 덮는 계산 · 프레임별 제 크기 · 끄는 시점은 그대로. 로그 `on` 줄에 `overscan N.NN` |

- 처음(`47f53c8`)엔 위 추정 최소값을 모두 넘는 1.12 를 잠정값으로 넣었다. **Play 에서 사용자가 1.08 로 골랐다**(아래) — 추정은 95% 덮임 기준이라 남는 5% 는 눈에 띄지 않았다. 1.08 이면 좌우 각 약 77 UI px(화면 너비의 4%)가 더 잘린다.
- 비교용 키 토글(DevTest · 숫자 1~4 → 1.00 / 1.04 / 1.08 / 1.12)은 로컬 테스트 브랜치에만 넣었고 이 브랜치에는 없다.
- **임시값이다.** 디자이너가 궁 컷신을 새로 만들어 넘겨줄 예정이라(사용자 2026-09-26) 그때 테두리 처리에 맞춰 다시 정한다. 전체 화면 UI 그룹(`SkillCutsceneGroup` · HUD 위 · 시전자만)은 그대로 쓴다.

### 검증

- LSP 진단: 에러 0 · 경고 0. `check-integrity`: 통과.
- **Maker Play (2026-09-26 · 로컬 테스트 브랜치 `c4d35ce` = origin/main + #83 `7078f3e` + #98 `7001307` + #84 `47f53c8` + 토글 · Reimport All · Maker MCP):**
  - 빌드: 오류 0 · 경고 1(`LWA-1111` · 기준선과 같음 · 1 → 1). 런타임 오류 = 로비에서 매치를 시작한 하네스의 `[BalrogRoom] 방N 스포너 없음` ×6 뿐 · 경고 9 는 이 변경 밖(`[BossCatalog]` ×4 · `LWA-3047` ×3 · `LWA-3048` ×2).
  - 준비(메모리만): `_MatchSessionLogic:StartMatch` · Lv30 · 3차 · 전 스킬 · 직업 무기 · 궁 5종 사용 제한 0 · 쿨타임 0 · 퀵슬롯 1·2 비움 · **맵 왼쪽 끝**(x −8.84 ~ −10.1 · 카메라가 멈추는 자리).
  - 전사로 사용자가 1.00 / 1.04 / 1.08 / 1.12 를 컷신 도중에 바꿔 가며 비교(로그 `[DEV] cutscene overscan = … (live rect …)`) → **1.08 선택**("1.08 부터 가득 참"). 가장자리뿐 아니라 인물 · 얼굴 같은 중요한 그림이 잘리지 않는지도 봤다.
  - 1.08 로 나머지 넷을 한 직업씩 전직해 시전 → 사용자 판정 5종 모두 **가득 참**:

| 궁 | 로그 (`cutscene UI … on`) | 사용자 판정 |
|---|---|---|
| 불굴의 진 SK_W31 | `clip 1436x840 … rect 2074x1213 overscan 1.08 for 5.34s` | 가득 참 |
| 대마법 SK_M31 | `clip 1456x860 … rect 2074x1225 overscan 1.08 for 5.52s` (frames min 480x820 = 시작 섬광) | 가득 참 |
| 폭풍의 화살 SK_A31 | `clip 1456x860 … rect 2074x1225 overscan 1.08 for 7.98s` (추정 최소 1.11) | 가득 참 |
| 메소 익스플로전 SK_T31 | `clip 1456x860 … rect 2074x1225 overscan 1.08 for 5.13s` (메소 동전 15 개 · `coins=15 -> 750%`) | 가득 참 |
| 함포 사격 SK_P31 | `clip 1444x844 … rect 2074x1212 overscan 1.08 for 4.89s` (못 잰 클립 · 눈으로만) | 가득 참 |

  - 모든 시전이 실측 길이 뒤 `cutscene UI … off`. 컷신 도중 같은 궁 재시전(전사 `on` 12:34:27 → `on` 12:34:31) → 이전 끄기 타이머는 무시되고 새 시전 길이 뒤 `off` 한 번(12:34:37).
