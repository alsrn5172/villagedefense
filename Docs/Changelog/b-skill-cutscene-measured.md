...
| 파일 | 변경 |
|---|---|
| `SkillExecutors.mlua` | `property number CutsceneOverscan = 1.12` — `StartCutsceneUI` 가 덮는 배율(`max(가로비, 세로비)`)에 곱한다(가운데 기준 · 네 변 같은 비율 · 0 이하면 1). 덮는 계산 · 프레임별 제 크기 · 끄는 시점은 그대로. 로그 `on` 줄에 `overscan N.NN` |

- **1.12 는 잠정값**이다 — 위 추정 최소값을 모두 넘는 후보(1.00 / 1.04 / 1.08 / 1.12) 중 가장 작은 값. 1.12 면 좌우 각 약 115 UI px(화면 너비의 6%)가 더 잘린다. Play 에서 눈으로 골라 바꾼다.
- 비교용 키 토글(DevTest · 숫자 1~4 → 1.00 / 1.04 / 1.08 / 1.12)은 로컬 테스트 브랜치에만 넣고 이 브랜치에는 넣지 않는다.

### 검증

- LSP 진단: 에러 0 · 경고 0. `check-integrity`: 통과.
- **Maker Play: 아직** — 로컬 테스트 브랜치(#83 + #98 + #84 + 토글)에서 사용자가 값을 고른다.
