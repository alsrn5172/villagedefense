...
### Play 검증 6차 (대기 · 재입장 + Reimport All 뒤)

| 항목 | 기대 | 결과 |
|---|---|---|
| 빌드 경고 | 1 → 1 | 대기 |
| 한손 · 두손 청보라 잔상 | 알파 0.75 로 드러남 · 접촉 프레임에 · 0.35s | 대기 |
| 위치 swingO2 · O3 · T2 · T3 (좌우) | 원화와 같은 자리(O1 · T1 은 메모리 확인 완료) | 대기 |
| 스폰 첫 프레임 | 잔상 · 화살 · 표창 모두 에너지볼트 공이 안 보인다(프레임 관찰) | 대기 |
| 에너지볼트(SK_M11) | 공 모습 그대로 | 대기 |
