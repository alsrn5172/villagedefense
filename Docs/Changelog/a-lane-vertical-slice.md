...
- (저녁) §1·§13 에 "로비 / 결과 화면·관전은 **안 만들었다 — 기획부터**"(있는 것 = `.ui` 껍데기 + 문자열 그리는 컨트롤러) 명시, §2-1 표에 8 로비 기획 · 9 결과 화면 기획 · 10 관전 기획 행 추가. 세 화면을 샘플 값으로 띄운 스샷 `메월드폴더/lobby.png · result.png · spectate.png` 를 사용자에게 전달(문서·스샷만 · 코드 변경 없음).
