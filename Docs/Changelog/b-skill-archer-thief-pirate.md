...
- **또 제보 "한 번 들기는 너무 미묘 · 공격 동작은 말고 조금 더 눈에 띄게"** → 시전 행을 `heal` **1.5배속 ZigzagLoop**(팔을 위아래로 계속 흔드는 동작)으로 바꿔 시전 락 동안 돌리고, `_2` 빠른 전투 자세는 하이퍼 바디·도발 1.0s · 불굴의 진 1.5s(락 끝), `_END` 1.6s/1.6s/5.5s. 사용자가 직접 Play 로 확인하기로 함(요청 시에만 MCP 검증) — 이 항목은 **Play 미검증**.
...
- 전엔 "8초 무적"(IsInvincible → 피격 자체 없음 · A 의 PlayerHit 연결 대기라 실제론 아무것도 안 막혔다). 이제 **피격은 등록되되 HP 만 최대치로 고정**되고, 경직·넉백이 없으며, 피격 무적 시간이 절반이라 아이언 바디 반사가 두 배로 자주 난다.
- **`RootDesk/MyDesk/PlayerHit.mlua`(루트 · 스킬-모션-구현맵 §1 "공격 파이프라인 (B 담당)" 의 `PlayerHit.mlua | 피격 | B`)** 를 처음 편집했다(저장소 초기화 뒤 아무도 손대지 않은 MSW 기본 파일). ⚠ 구현항목-결정 §3 는 담당자를 "등록서 8번" 으로 두었으므로 A 확인 요청.
  - `IsHitTarget`: `ImmuneCooldown`(1s) × `_SkillBuffs:GetHitImmuneMul(uid)`(불굴의 진 = 0.5) → 무적 시간 절반. 서버에서만(`self:IsServer()`), 시그니처·ExecSpace 는 부모 그대로(LEA-3014).
  - `OnHit(attacker, damage, isCritical, attackInfo, hitCount)` 오버라이드(msw-combat-system §2 시그니처): 서버에서 `_SkillBuffs:IsUnyielding(uid)` 이면 **`__base:OnHit` 을 건너뛰고** `OnUnyieldingHit` 만 부른다 → HP 감산·HIT 상태(경직·넉백)·피해 표시·HitEvent 전부 없음. 아니면 엔진 기본 동작 그대로(다른 버프의 자가 배선도 그대로 — `ModifyIncomingDamage` 정식 연결은 이 오버라이드에 한 줄 추가로 가능하나 이번 범위 밖).
- `SkillBuffs`: `IsUnyielding`·`GetHitImmuneMul`(ExecSpace 없음 · IsGuaranteedCrit 와 같은 규칙) · `OnUnyieldingHit`(HP 최대치 + 아이언 바디 반사 = 새 `ReflectToAttacker` · OnPlayerHitEvent 도 같은 함수를 쓴다) · `StartUnyielding/StopUnyielding`(버프 시작 시 HP 최대치 + `UnyieldingHpTick` 0.1s 반복 타이머로 재고정 — PlayerHit 을 거치지 않는 HP 감소도 덮는다 · 버프 종료/만료 시 타이머 해제). 상수 `UnyieldingImmuneMul` 0.5. 태그 `INVULNERABLE` 은 계약 그대로(이름만 옛 의미).
- `SkillInfo.csv` `SK_W31` Description·#Note 만 변경(값 불변 · Duration 8).
- Play 확인(한손검 · 주황버섯 위에 서서 · 몹 HP 는 테스트로 크게): **버프 없음** = 3초에 2회 피격(`HPCHECK base-before hp=201450` → `base-after hp=201448` · 각 1 피해 · 반사 50362 ×2) · **R 뒤** = `[Buff] ON INVULNERABLE … dur=8` · `UNYIELDING hp pinned to max, immune x0.5, no stun/knockback` · `HPCHECK … unyield=true mul=0.5` · 피격마다 `UNYIELDING hit ignored dmg=1` + `IRON_BODY reflect 50362`(OnUnyieldingHit 경로) · HP 는 내내 `201450/201450` · 8초 뒤 `OFF INVULNERABLE` · `unyield=false mul=1`. 스크린샷: 버프 중 피격 순간에도 피격 자세·붉은 깜빡임·밀림 없이 전투 대기 자세 그대로. 빌드 오류 0(오버라이드 시그니처 통과).
- **무적 시간 절반 실측**(반사가 몹을 밀어내 붙어 있지 못하므로 `_SkillBuffs.ReflectEnabled=false` 를 Play 메모리에서만 끄고 4초씩 붙어 있기): 버프 없음 = 4초에 피격 1회(HP −1 · 무적 1s) · 버프 중 = 4.5초에 `UNYIELDING hit ignored` **6회**, 같은 초 안에 2회씩 두 번(13:48 ×2 · 13:52 ×2 = 간격 ≤1s → 무적 0.5s 가 아니면 불가능) · HP 내내 최대치.
- **사용자 요청 재검증(2026-09-13 · 서버 50ms 샘플러 = `PlayerHit.LastHitTime`·HP·x·StateComponent 상태를 타이머로 로그 · 반사 OFF · 주황버섯 옆에 서기)**:
