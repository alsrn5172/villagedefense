...
- `Skill/SkillCaster.mlua`
  ...
  - 신규 `ReportCastLockDeath(castId)`(Server · 관전자 차단): 서버에서도 HP 0 이하(또는 사망)일 때만 · 그 시전자의 스킬 연출 엔티티 `SkillFx_<uid>_*`(휘두르기 잔상 등)를 지우고 1.5s 동안 0.1s 마다 다시 쓸어낸다(늦게 스폰되는 연출까지).
    - 2026-09-25 추가: 그 1.5s 안에 부활하면(부활 팝업에서 곧바로 고르기 등 · 서버 `Hp > 0` 이고 `IsDead()=false`) 쓸기를 곧바로 멈춘다 — 부활 직후 새로 시전한 스킬의 연출을 지우지 않게. 로그 `sweep stopped (revived)`.
...
| 항목 | 기대 | 결과 |
|---|---|---|
테스트 빌드 = 이 브랜치 + #83(`b/skill-powerstrike-mul`)을 합친 로컬 전용 브랜치(푸시 안 함) — 0.45s 대기는 #83 의 파워 스트라이크 타이밍이다(main 은 0.2s · 0.75s 2단). 고칠 것은 이 브랜치에만.
...
| 사망은 한 번만 | 사망 1건 = `ProcessDead` 1번 · 부활 대기 시간(RespawnDuration) 그대로 · A `[Death]` 사망 횟수 +1 · 페널티 1번 | 대기 |
| 부활 직후 시전 | 1.5s 쓸기 안에 부활해 시전해도 그 연출이 남는다(`sweep stopped (revived)`) · 스킬 정상 | 대기 |
