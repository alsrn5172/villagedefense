...
**예열 순간 프레임(차가운 입장)**: 입장 3s 뒤 예열(`cutscene prewarm x8`) 직후 약 2s 동안 끊김 — 258 · 281 · 285ms 멈춤 3번 + 50~110ms 프레임 약 10개 · 앞뒤는 17ms. **원인은 이 PR 이 아니다**: 같은 방식으로 보낸 차가운 작은 클립(11001002 cast 8프레임)은 끊김 0 · 뒤에 로드 16ms(이미 받음) → 하이퍼 바디 리마스터(17프레임 · 작은 클립)도 같은 부류. 끊김은 main 에 이미 있던 화면 컷신 5종(≈1400×850 · 79~133프레임) 예열 몫이다. 줄이는 작업은 #95 뒤 별도 B PR(현재 직업 궁 컷신만 전직 뒤 예열 + `_ResourceService:PreloadAsync` 시험).
