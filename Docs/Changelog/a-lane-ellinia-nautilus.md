...
| ELLINIA | Ellinia_Village_MinimiMain | VILLAGE | −4.0 ~ 4.1 | 6.8 | −3.4 → 4.1 | 넥서스 3.7 (2층 y 1.06) | **새 발판 없음** — 미니언은 4층 둥근 다리(5.6~6.8)를 걷다 오른쪽 끝(3.28)에서 떨어져 2층 넥서스를 친다 (사용자 2026-09-22) |
...
- 엘리니아 넥서스(사용자 2026-09-22): "넥서스는 2층 · 미니언은 둥근 길" → 넥서스 (3.7, 2층 y 1.06). 미니언 AI 는 가로·세로 합친 직선거리로 목표를 잡아(탐지 6 · 공격 1.1) 다리 위에서 넥서스를 발견하면 그쪽으로 걸어가 다리 끝에서 떨어진 뒤 친다. Nest2 카메라 경계는 사용자가 직접 설정(`UseCustomBound` · (−2.25, −6.27) ~ (3.15, 20.15)).
