...
|---|---|---|---|---|---|---|---|
| ELLINIA | Ellinia_Hunt_TreeTrunkNest2 | LANE2 | −2.2 ~ 3.1 | −5.65 | −1.6 → 2.7 | 포탑 0.45 | **사용자가 Maker 에서 통로를 구덩이 안으로 옮김**(2026-09-22 · 바닥 한 장 · 사다리 4칸 −0.77 / 1.7 · 통로 포탈 2.55) |
| ELLINIA | Ellinia_Hunt_GreenTreeTrunk | LANE1 | −17.3 ~ −1.6 | −20.7 | −16.7 → −2.0 | 억제기 −6.9 | 사다리 −9.5(4칸) / −4.5 |
| ELLINIA | Ellinia_Village_MinimiMain | VILLAGE | −4.0 ~ 4.1 | 6.8 | −3.4 → 4.1 | 넥서스 −0.5 (2층 y −0.17) | **새 발판 없음** — 미니언은 4층 둥근 다리(5.6~6.8)를 좌→우로 걷다 오른쪽 끝(3.28)에서 2층으로 떨어져, 왼쪽으로 걸어가 x축 가운데 넥서스를 친다 (사용자 2026-09-22) |
...
- Nest2(사용자 2026-09-22 Maker 수정): 통로 바닥을 `LaneGround_0` 한 장으로 줄여 구덩이 안 y −5.65(x −2.28~3.17 · Maker 가 구움)로 올리고 `LaneGround_1/2` 삭제 · 사다리를 4칸으로 −0.77 / 1.7 · 통로 포탈 (2.55, −5.59) · 여섯갈래길 포탈 −1.72 · 나무 발판 장식 소폭 이동 · 타일 35칸 · `TileMap_1.IsBlockVerticalLine` 켬. 남겨진 사다리 받침 2개만 새 몸통 아래로 붙였고, `LaneConfig` 를 새 통로에 맞췄다(포탑 0.45 = 두 사다리 사이).
- 엘리니아 넥서스 최종(사용자 2026-09-22 "x축 기준 중앙 · 2층 유지"): (−0.5, y −0.17) = 마을 바닥 −5.15~4.16 의 가운데 · 2층 낮은 길. 넥서스 윗부분(3.13)이 바로 위 가운데 발판(3.2) 밑에 들어간다.
  - 🔧 `FactionAI.MaxTargetDy = 2.5` 신설: 세로로 2.5 넘게 떨어진 대상은 목표로 잡지 않는다. 안 그러면 다리 위 미니언이 넥서스 바로 위(높이 차 4 이상)에 멈춰 영영 못 친다. 같은 층 시설은 중심이 바닥 + 1.1~1.8 이라 영향 없음 — 모든 미니언·수비대에 적용.
- (중간안) "넥서스는 2층 · 미니언은 둥근 길" → 넥서스 (3.7, 2층 y 1.06). 미니언 AI 는 가로·세로 합친 직선거리로 목표를 잡아(탐지 6 · 공격 1.1) 다리 위에서 넥서스를 발견하면 그쪽으로 걸어가 다리 끝에서 떨어진 뒤 친다. Nest2 카메라 경계는 사용자가 직접 설정(`UseCustomBound` · (−2.25, −6.27) ~ (3.15, 20.15)).
...
- 엘리니아 다리는 발판 그룹 하나(−3.1 ~ 3.28 · Prev/Next 링크 연결)와 왼쪽 끝 조각(−4.04 ~ −2.94 · 0.02 아래 겹침)으로 돼 있다. ~~`FactionAI` 는 발판 끝에서 멈추지 않으므로 0.02 떨어져 이어 걷는다~~ → **틀렸다**(2026-09-22 실측): 몬스터 Rigidbody 는 `IsolatedMove` 라 체인 끝에서 멈춘다. 아래 2026-09-22 절 참고.
...
- ↔ 좌→우 전환 뒤 재검증: 아래 2026-09-22 절(Play 5회).

## 2026-09-22 — 발판 교차 자동 전환(#19 흡수) · 엘리니아 NPC 3·4·5층 · Maker 실측

### 엘리니아 마을 기능 NPC (사용자 "npc들 적당히 3층 4층 5층에 배치해라")

- `VillageNpcSector.csv` ELLINIA 4행만: WORKSHOP 3층 가운데 (−0.8, 3.18 · 간격 0.8) · LIFE 4층 다리 가운데 (−0.6, 5.65 · 0.6) · DEFENSE 4층 다리 오른쪽 (2.4, 6.78 · 0.6) · RECORD 5층 윗가지 (−2.2, 7.16 · 0.6). AnchorY = 바닥 y (헤네시스와 같은 규칙).
- 실측: 10명 전부 그 자리 · 발밑 바닥과 차이 0.03 이하.

### 발판 교차 자동 전환 — #19 흡수 (사용자 "#19는 버리고 거기 있는 내용만 가져와서 여기서 같이 점검 · 맵도 바꾸는데 로직을 같이")

- #19(빈 락 커밋뿐)는 닫고 락은 이 PR 본문으로 옮겼다.
- **원인** (전 맵 스캔 + 실측): 가져온 메이플 맵은 장식 오브젝트마다 발판 체인이 따로고, 엔진의 걷기는 자기 체인의 Prev/Next 링크만 따라간다.
  - 체인 끝 바로 앞에 조금 높은 다른 체인(이음매)이 있으면 **플레이어는 그 밑으로 떨어지고**(엘리니아 3층 → 2층 y 0.14 실측) **몬스터는 멈춘다** — 몬스터 Rigidbody 는 `IsolatedMove = true`(체인 끝에서 안 떨어짐 · 플레이어는 false · 실측). 전 맵 이음매 335곳(오를 높이 0~0.6 · 0.3~0.6 이 130곳).
  - 바닥의 `IsBlockVerticalLine` 은 엔진에선 **자기 체인의 벽만** 막는다 — 다른 오브젝트의 벽은 켜도 통과.
  - 정확히 45° 인 선분도 몬스터에겐 벽이다(엘리니아 다리 77번 · 실측). 체인 안의 51° 오르막은 엔진이 그냥 올려 줬다.
- 새 `Map/FootholdSwitch.mlua` (@Logic `_FootholdSwitch` · 새 폴더 `Map/`):
  - **R1 이음매**: 걷던 체인이 끝나는 곳 앞 0.05 에 0.6 이하로 높은 바닥이 있고 0.35 안쪽 아래에 바닥이 없으면 그 위로 옮긴다. 발 위로 0.6 넘게 솟은 벽 너머로는 안 옮긴다. 플레이어 = 클라이언트(이 Logic 의 OnUpdate · 로컬 플레이어만), 몬스터·미니언·수비대 = 서버(`Monster.OnUpdate` 가 `TryStepUp`). 감지 거리 = max(0.1, 프레임당 이동 × 2).
  - **R1b**: 체인 끝 앞이 다른 체인의 45° 이상 내리막이면(전 맵 7곳) 내리막 아래 첫 완만한 선분으로.
  - **직후 건지기**(플레이어): 프레임이 튀어 끝을 지나쳐 떨어지면 끝에서 R1 이 쓰려던 **그 발판**으로만 올린다(처음엔 떨어지는 자리에서 새로 찾아 위 가지로 끌어올렸다 → 고침 · 실측).
  - **R2 벽**(플레이어만 · 옵션): 밟은 바닥 발판의 `IsBlockVerticalLine`(Maker 에서 그 바닥 오브젝트의 `CustomFootholdComponent` 에 켠다) 또는 플레이어 Rigidbody 의 `IsBlockVerticalLine` 이 켜져 있으면, 어느 오브젝트 것이든 발 높이에 서 있는 벽을 넘는 순간 벽 앞으로 되돌린다(같은 x 에 이어진 벽 조각은 합쳐 본다). **꺼진 바닥은 지금처럼 통과** — 헤네시스 마을은 건물 벽 4개 사이를 지나다니는 설계(NPC 자리)라 전부 막으면 안 된다. 입력을 끊는 방식(MoveVelocity 0)은 컨트롤러가 곧바로 다시 써서 효과가 없었다(실측).
  - 로그 `[FootholdSwitch] step/slope/catch/drop/wall …` — 서버·클라이언트 각각 처음 40줄만.
- `Monster.mlua`: OnUpdate 앞에서 `TryStepUp`(넉백·사망 중 제외 · 옮긴 뒤 0.2초 쉼). `StateTypeWander.mlua`: 체인 끝이어도 `HasStepAhead` 면 계속 걷는다.
- `Lane/MinionUnit.mlua` `KeepMoving`: ① 전진(ADVANCE) 중 체인 끝에 0.4초 막히면 `TryDropOff` 로 끝 너머 0.1 로 내려준다(구간 끝 EndX 에 이미 닿았거나 · 아래 15 안에 발판이 없으면 안 함 — 넥서스가 부서진 뒤 맵 끝에서 허공으로 떨어지지 않게) ② VILLAGE 역할이 통로 높이보다 **2.5**(= `FactionAI.MaxTargetDy`) 넘게 내려오면 넥서스(TowerSlotX) 쪽으로 방향을 돌린다. FactionAI 의 전진은 통로 방향으로만 걷기 때문에, 엘리니아처럼 넥서스가 통로 선 위에 없는 마을은 이게 있어야 넥서스에 닿는다. 기준을 1 로 뒀을 땐 다리가 가운데서 1.16 처져 다리 위에서 왔다갔다했다(실측).
- 🐞 정정: 넥서스 2층 안의 "미니언이 다리 끝에서 떨어져 왼쪽으로 넥서스까지 간다"는 설명은 **코드상 성립하지 않았다**(끝에서 안 떨어짐 · 전진은 오른쪽 고정). 위 KeepMoving 으로 실제로 그렇게 가게 만들었다.
- Codex 검토(terra xhigh · 새 세션 2회 · 읽기 전용) — 두 번 다 같은 4건을 짚었고 전부 반영:
  - R2 벽 목록이 45° 이상 경사까지 벽으로 넣음 → 벽은 **약 80° 이상 세운 발판**(`IsVerticalWall`)만. 45° 기준(`IsWall`)은 바닥 판정에만 쓴다.
  - R1b 가 첫 가파른 선분이 진행 방향 **내리막**인지 안 봄 → 거쳐 가는 선분이 전부 내려가야 하고 세운 벽이면 포기 · 착지점이 끝보다 낮아야 한다.
...
- 플레이어(엘리니아 마을):
  - 3층 이음매 (0.07, 3.03) → 위 0.18: 켜면 `step (0.17,3.08) -> (0.02,3.22)` 후 3층 (−0.65, 3.19) 에 남음 / `MaxRise = −1` 로 끄면 같은 걸음에 2층 (−1.63, 0.14) 으로 떨어짐
  - 2층에서 프레임이 튄 경우 `catch (−2.98,1.07) -> (−2.98,1.27)`
  - 다리 45° 구간 `slope (−3.07,6.68) -> (−2.69,6.32)` 후 다리를 따라 (−1.48, 5.99)
- 벽(헤네시스 마을 x −1.73 · 바닥 위 4.46 높이): 플레이어 Rigidbody 옵션 켬 → `wall x=−1.69 -> −1.76` 벽 앞 정지(y −0.04 유지) / 끔 → 통과해 0.11
- 몬스터: 커닝 공사장 야생 몹이 이음매를 건너감(`step Monster_AbandonedConstructionSite_*` 서버 로그)
- 미니언(점유 = `ClaimLevel=1` + 가짜 유저 · HP 5000):
  - 엘리니아 마을: `slope (−3.04,6.67) -> (−2.69,6.32)` → 다리 오른쪽 끝 (3.23, 6.80) → `drop (3.28,6.80) -> (3.38,6.81)` → `left the village path … head to nexus x=−0.50 dir=−1` → 2층 (0.92, 0.05) 에서 넥서스 `ATTACK` 반복
  - 나머지 5구간(엘리니아 LANE2·LANE1 · 노틸러스 LANE2·LANE1·VILLAGE): 좌→우로 걸어 포탑(−0.1) · 억제기(−8.1) · 포탑(−7.7) · 억제기(−2.74) · 넥서스(19.18) 앞에서 공격. y 변화·추락 없음 — KeepMoving 추가 전후 같음
  - 에러 = `LEA-3032` 가짜 유저 `T_ELL`/`T_NAU` 뿐(테스트 전용)
- 눈 확인(사용자 몫): 엘리니아 NPC 3·4·5층 자리 · 이음매 올라탈 때 모습 · 벽 옵션을 켤 바닥 선택
