...
킹슬라임 · 스텀피 · 피아누스 · 발록(박스만) · 마노(돼지해변·여섯갈래길 2곳).

**예외 2곳 (사용자 지시로 손대지 않음)**
- `Ellinia_Hunt_GiantTree` → 에피네아
- 🔴 `Henesys_Hunt_BlueMushroomTrail` → **머쉬맘** — 한 번 빨강으로 바꿨다가 **바이트 단위로 원상복구**했다
  (`edd691cd` · 박스 `0.44×0.44` / offset `0,0`). 머쉬맘 쪽은 좀비머쉬맘 점검 중이라 건드리지 않는다.
