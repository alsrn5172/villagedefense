...
`Ellinia_Hunt_GiantTree` → 에피네아만 **사용자 지시로 예외 유지**.

> 중간에 머쉬맘 입구를 한 번 되돌렸다가(`819a9a7`) 사용자 확인 후 다시 적용했다(`재적용`).
> 아래 ⑤ 의 "스킬 4행 고아" 를 내가 만든 문제로 오해하게 쓴 탓이다 — 포탈 통일 자체는 처음부터 맞았다.
...
사용자 확인: **의도한 것 · 그대로 둔다.** 표(`BossInfo`/`BossSkills`)·보스맵·입구 포탈 **전부 무변경**이다.
...
- ⚠️ **머쉬맘 스킬 4행이 참조를 잃은 상태다** (이번 작업이 만든 게 아니다).
  `BossSkills` 에 `BossId=6130101` 행이 4개 남아 있는데 `BossInfo` 의 머쉬맘이 `Enabled=false` 라
  `BossCatalog` 가 로드할 때마다 `skill points to unknown/disabled BossId=6130101` 경고를 4번 낸다.
  **원인은 커밋 `ad094e4`(머쉬맘 비활성)이고 그건 사용자가 의도한 상태다.** 동작에는 영향이 없다 —
  머쉬맘이 안 뜨니 그 스킬도 쓸 일이 없다. 나중에 머쉬맘을 다시 켜면 경고는 저절로 사라지지만,
  그때 좀비머쉬맘과 **좌표가 `(-6,-1)` 로 정확히 겹치는 것**은 따로 정리해야 한다.
