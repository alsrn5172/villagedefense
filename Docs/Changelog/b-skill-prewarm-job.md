...
1. **지금 직업 것만 · 전직 뒤에**(사용자 결정 ①):
   ...
   - `PrewarmRuidsFor` = 그 직업(`SkillInfo.ReqJob`) 스킬의 `cast.cutsceneSeconds`(= 궁 화면 컷신) · `cast.prewarm == true`(#94 하이퍼 바디 리마스터) · `thrust`(main 의 파워 스트라이크 2단 잔상 · #83 이 지우면 저절로 빠진다). 같은 RUID 는 한 번.
