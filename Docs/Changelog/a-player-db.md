...
**규칙 5의 "잠긴 매치 참여 거절"만 실측을 못 했다.** 주인이 [시작하기] 를 눌러 매치가 잠기는 **바로 그 순간**에
다른 사람이 [참여하기] 를 눌러야 재현되는데, 손으로는 타이밍을 맞출 수가 없다.

- 지금까지 **한 번도 발생하지 않았다.** 사용자 가상플레이에서도 안 났다
- 서버 분기는 있다 — `RequestJoinMatch` 가 `m.locked` 를 보고 토스트 + 목록 재푸시로 거절한다
- 잠기는 즉시 목록에서 빠지므로(`LockMatch` → `PushBrowse`) 클라가 그 매치를 고를 수 있는 창 자체가 아주 좁다
