...
### 2026-09-13 — 제보 반영 ⑤ 분신은 플레이어 뒤 층에 · ⑥ 표창 사거리 재확인(아래층 제외)

- 제보(스크린샷): "오른쪽(앞)을 볼 때 분신이 앞을 가린다" · "표창 사거리가 표대로인가? 2층에서 던졌는데 아래층 몬스터도 죽었다".
- ⑤ 원인 둘: (a) 이펙트 기본 층(Default)은 플레이어 층 위라 분신이 **캐릭터 앞에** 그려졌다. (b) 던지기 클립 `special/swingT3` 의 프레임 pivot 이 (39,−4)/(44,−4)로 뒤쪽에 있어(stand1 은 (28,−4) 가운데) 팔이 발보다 **44px 앞**까지 뻗는다 → 오른쪽을 볼 때(FlipX) 분신 발이 −45px 이면 팔 끝이 캐릭터 중심(−1px)까지 와서 몸을 덮었다. 왼쪽을 볼 땐 뒤집지 않아 +1~+61px 로 안 겹쳤다(제보의 "앞을 볼 때" = 오른쪽).
  - 고침: `ApplyShadowSorting` — 분신 루프·따라하기 클립을 **플레이어 렌더 층 뒤**에 그린다(원작 = 분신은 캐릭터 뒤). 플레이어 렌더 층 = 밟은 발판의 SortingLayer + OrderInLayer 4(A 의 `Npc/NpcSpawner`·`PortalNetwork` 주석 · NPC 3 · 몬스터/보스 2) → `_LaneFacilityService:SortingLayerBelow(map, x, y)` 층 + `ShadowOrderInLayer` **2** 를 `PlayEffectAttached` options(`SortingLayer`/`OrderInLayer` · `EffectService.d.mlua`)로. 발판을 못 찾으면(공중 · 발 아래 3유닛 안에 없음) 옵션 없이 예전 층. spec `loop.behindPlayer = true` 로 켠다(다른 루프는 그대로). 오프셋 −0.45 는 원작(30~40px 뒤)에 맞춰 유지 — 팔이 겹쳐도 뒤에 그려진다. 로그 `shadow mimic swingT3 facing=1 layer=MapLayerN/2`.
