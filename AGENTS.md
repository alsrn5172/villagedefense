...
## O-5. 🔴 스킬 · 공격 모션을 구현하기 전에 구현맵을 읽는다

**스킬 · 플레이어 공격 · 무기 모션과 관련된 작업은 `Docs/스킬-모션-구현맵.md` 를 먼저 읽는다.**

거기에 (a) 어느 파일에 뭐가 구현돼 있는지, (b) 무엇이 런타임 검증됐고 무엇이 추측값인지,
(c) B 가 표만 채우면 되도록 만들어 둔 인계 방법이 있다.

읽지 않으면 반복하기 쉬운 실수:

- **"아바타 아이템 카테고리만 맞추면 무기 모션이 따라온다"** → 아니다. MSW 는 활을 끼워도 활 쏘기를 자동
  재생하지 않는다. 카테고리는 필요조건일 뿐이고, `shoot1` 같은 액션은 스크립트가 직접 재생해야 한다
  (2026-09-08 실제로 이걸 틀려서 "활이 검처럼 휘둘러지는" 상태로 넘어갔다)
